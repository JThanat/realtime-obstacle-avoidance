...
// epipolar geometry constraint: m2^t*F*m1=0
    ...
    if( fs.isOpened() )
    {
        ...
        fs << "VROIX" << validRoi[1].x << "VROIY" << validRoi[1].y << "VROIW" << validRoi[1].width << "VROIH" << validRoi[1].height;
        fs.release();
    }
    else
...
{   
    ...
    string imagelistfn;
    bool showRectified;
    cv::CommandLineParser parser(argc, argv, "{w|9|}{h|6|}{s|1.0|}{nr||}{help||}{@input|../calib_img/stereo_calib.xml|}");
    if (parser.has("help"))
        return print_help();
