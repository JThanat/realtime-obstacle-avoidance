    ...
    vector<iPair> waypoint_checking(vector<iPair> waypoints, vector<ellipse_desc> &obstacles, int obstacle_count)
    {
        ...
        for (i = 0; i < waypoints.size(); i++)
        {
            ...
            while (!wps_to_check.empty())
            {
                ...
                {
                    k = 1;
                    while(has_collision(make_pair(current_check_point.first - 50*k, current_check_point.second), obstacles, obstacle_count))
                    {
                        k++;
                    }
                    wps_to_check.push(make_pair(current_check_point.first - 50*k, current_check_point.second)); // path to left
                    k = 1;
                    while(has_collision(make_pair(current_check_point.first + 50*k, current_check_point.second), obstacles, obstacle_count))
                    {
                        k++;
                    }
                    wps_to_check.push(make_pair(current_check_point.first + 50*k, current_check_point.second)); // path to right 
                }
                else
                {
            ...
            selected_index = 0;
            min_dist = cur_dist + distance(allowed_wps[i][0].first, current_point.first, allowed_wps[i][0].second, current_point.second);
            ...
            for( j = 0 ; j < allowed_wps[i].size() ; j++)
            {
                ...
                if (min_dist > (cur_dist + distance(allowed_wps[i][j].first, current_point.first, allowed_wps[i][j].second, current_point.second)))
                {
                    min_dist = cur_dist + distance(allowed_wps[i][j].first, current_point.first, allowed_wps[i][j].second, current_point.second);
                    selected_index = j;
                }
            }
            cur_dist = min_dist;
